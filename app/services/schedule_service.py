...
def suggest_schedules(
    ...
    """Sugere horários com base no histórico do cliente e disponibilidade atual."""
    ...
    history = schedule_repository.list_schedules_by_client(db, client.id) if client else []
    ...
    for (preferred_weekday, preferred_time), stats in ranked_preferences:
        ...
        while candidate_date <= max_date:
            ...
            if pair not in seen_pairs:
                is_available = (
                    validate_working_hours(db, candidate_date, preferred_time)
                    and not schedule_repository.check_conflict(db, candidate_date, preferred_time)
                )
                if is_available:
                    seen_pairs.add(pair)
                    suggestions.append(
    ...
    return {"client_name": client_name, "suggestions": suggestions[:limit]}
